                        ...
                        <tbody>
                            @foreach ($productos as $producto)
                                <tr class="border-b hover:bg-gray-50">
                                   {{-- $loop->iteration --}}
                                    <td class="py-4 text-center">{{ $producto->id }}</td>  
                                    <td class="py-4 text-center">{{ $producto->nombre }}</td>  
                                    <td class="py-4 text-center">{{ $producto->descripcion }}</td> 
                                    ...
                                    <td class="text-right"> 
                                       <div class="d-flex flex-row bd-highlight mb-3 " style="display: flex; justify-content: flex-end">
                                           ...
                                           <div class="p-2 text-right bd-highlight">
                                               <a href="{{ route('productos.edit', $producto) }}" class="btn-sm btn text-right btn-primary">
                                                   <i class="fas fa-edit"></i>
                                               </a>
                                           </div>

                                           <div class="p-2 text-right bd-highlight">
                                               <form action="{{ route('productos.destroy', $producto) }}" class="d-inline text-right formulario-eliminar" method="POST">
                                                   @csrf
                                                   @method('delete')
